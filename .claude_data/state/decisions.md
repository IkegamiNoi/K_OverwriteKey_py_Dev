...
## 2026-09-17〜18 (phase 20: フル表示ウィンドウの縦方向の最小サイズ・暫定 18)

### 【起票】暫定仕様 18（v0.5・ユーザー確定 2026-09-17）
- 暫定仕様先行 / 最小の高さ = 一覧を既定の半分の行数（6 / 6 / 9）にしたときの高さ / 高さは保存しない / 省略表示では解除。
- 起票時 deep-reviewer（実測: pack は後置から削り、expand の一覧は縮まない）→ **案 A = 一覧の `height` を 6/6/9 にし要求高さを最小に**（ユーザー選択。案 B = pack 順の変更は不採用）。
- 画面の高さ超過ははみ出しを受容（幅と同じ）。確定前 codex-adversarial（複数行の一時メッセージ）→ **一時メッセージは 1 行分で測り、最小付近の切れは受容**（ユーザー選択）。

### 【task_03 二次レビュー】deep-reviewer（修正要・中 1）+ codex-reviewer（指摘なし）→ ユーザー採否（2026-09-18）
- 指摘 1（中・メイン再実測で確認）: Tk が minsize で自動拡大した高さは geometry の記憶に残らず、最小が下がると 820 等へ縮む → **修正して採用**（task_03b: 測定直後に normal かつ最小未満なら geometry で最小へ）。
- 指摘 2（テスト: 拡大後に最小が下がっても維持）/ 3（枠内の子の実高さ ≥ 要求高さ）/ 6（変数名）→ **採用**（task_03b）。
- 指摘 5（`show_full_view` は `on_full_view_shown` を末尾へ移した = 推奨より広い）→ **採用・文書追従のみ**（task_04 の正本反映で記述を実装に合わせる）。
- 指摘 4（一時メッセージのラベルを App 属性で直接参照する案）→ **保留**（phase 01 で解消した「生やし」に戻るため）。
