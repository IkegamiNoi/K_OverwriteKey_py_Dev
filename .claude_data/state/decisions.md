...
## 2026-09-16 (phase 18: フル表示メイン領域の幅配分と境界線ドラッグ)

### 【task_02】Codex 利用上限 → **`implementer` で代替**（ユーザー許可 2026-09-16）

- 初回実装は `codex-implementer`。Codex 自身が確認 5（最小幅で中身が切れない）を未解決と申告
  （`pack` は先に詰めた部品へ要求幅を優先して配るため、一覧が先だとスクロールバー・ボタン列が削られる）。
- 追加修正（詰める順序の補正・`SASH_WIDTH` を `pane_width_rules.py` へ移しコントローラが View から import しない形へ）を
  Codex へ依頼したが**利用上限（解除 2026-09-17 0:40）**で実行不能 → **ユーザー許可のうえ Claude `implementer` で代替**。
- **ユーザー指示: フェーズの最終チェック（完了判定前の `codex-adversarial-reviewer`）は Codex が回復してレビュー可能になってから行う**
  （Claude 側だけへ縮退して完了判定しない）。

### 【task_05】二次レビュー指摘の採否（ユーザー 2026-09-17）

- **採用 → task_05b**: F1（= codex P2。ドラッグ後のウィンドウ最小幅を希望幅でなく表示幅から算出。task_03 の既知の残存を**受容せず修正**）/
  F2（`tests_ui` 3 モジュールが実 `config/config.json` を読み、目視でドラッグすると既定幅テストが落ちる）/ F4（起動経路での復元テスト）/ F5（`_on_press` でドラッグ状態を掃除）。
- **実機目視へ**: F3（最小幅での LabelFrame 見出しの切れ）/ F6（一覧の縦方向の広がり）。
- **除外**: F7（初回ドラッグで動かしていない側の既定幅も保存）= 暫定仕様 16 §3-6 どおり。task_06 の正本反映で文言を残す。

### 【運用】Codex 使用量の削減（ユーザー承認 2026-09-17）

- **実測**（`~/.codex/sessions` の `total_token_usage`）: 実装 / レビュー 1 回 = 12〜18 ターン・入力 56 万〜103 万（うち 80〜90% キャッシュ）・出力は数千。
  **主因は読んだ内容を毎ターン送り直す入力**。テスト実行（verifier = Claude 側）・推論強度（`low` 設定済み）は主因ではない。
- **読み込みを膨らませていたもの**: 転送文の「CLAUDE.md と .claude/rules/ に従う」（約 4 万字）/ `AGENTS.md` の「必須: codebase_map.md（4.6 万字）」/
  依頼文で手本に大きいテストファイルを丸ごと指定（task_04 で 7.8 万字）。
- **変更**: `codex-implementer.md`（3 変種）= 規約 3 ファイルのみ・読む範囲指定・全体読込は編集対象のみ /
  `task_new.md` に「読むファイル」節 / `AGENTS.md` と `implementation.md` = codebase_map は対象節のみ。
- **効果測定**: 次の実装タスクの Codex ログで `total_token_usage` を上記と比較する（見込み 3〜4 割減・推定）。

### 【task_05 実機目視 → 暫定仕様 16 v0.4】（ユーザー確定 2026-09-17）

- 目視 9「再起動するとトリガー一覧が意図した幅にならない」→ **案 A = フル表示時のウィンドウ幅 `full_view_window_width` を保存・復元**
  （ドラッグ時の書き込みに含める + 終了時）。目視「ボタン列がドラッグに巻き込まれる」→ **案 L = 移動イベントの間引き**（案 P プロキシ・案 S 枠外区画は不採用）。
- 敵対的レビュー 1 回目（高 1 / 中 1）: 終了時の保存は**フック停止後**・`destroy` を `finally` で保証（採用）/
  ウィンドウ幅だけ保存すると広げた分が出力シーケンスへ回る → Codex 案「希望幅も保存」ではなく**ユーザー案 = 既定幅を 780 基準で算出**。
- 再レビュー（中 1）: 狭い画面で起動して閉じるだけで保存幅が上書きされる → **アプリが自動で決めた幅は保存しない**（採用）。
  記録条件は「初回適用後は常に / フォント変更・フル表示復帰は geometry を変えたときだけ」（メインで明確化・ユーザーの縁リサイズを失わないため）。
- 目視 5（中ボタン）は機材なしで確認不可 → tests_ui のイベント生成で担保。目視外の気付き 2 件は idea_21（ヘッダ幅）/ idea_22（縦の最小サイズ）へ起票。
- 実装は task_05c（「読むファイル」節の初適用 = Codex 使用量の効果測定対象）。

### 【task_05c 実機目視 → 暫定仕様 16 v0.5】（ユーザー確定 2026-09-17）

- 目視: 全項目が規定どおり。ただし「幅を変える → 最大化 → 閉じる」で変えた幅が保存されない（v0.4 = 最大化中は書かない + 保存は終了時）。
  最大化前の幅は OS（Windows）が持ち、Tk からは取れない。
- 候補: A = `<Configure>` で通常時の幅を記録して終了時に保存 / B = Win32 `GetWindowPlacement` / **ユーザー案 = リサイズのたびに保存** → **採用（500ms 間引き）**。
- **終了時保存は廃止**（残す利点は間引き中に閉じた場合の取りこぼし防止のみ → 人の操作では起きないとユーザー判断・受容）。ドラッグ時の同時書き込みも廃止。
- 敵対的レビュー（高 1 / 中 1・裏取り済）: 保存失敗ダイアログ中にフックが止まらない（`startup_io.py:57-59`・既存の抜け）→ **`write_startup` 内で停止・finally 解除**（採用）/
  自動決定幅へ手で戻すと保存されない → **ユーザーが変えたら自動決定幅を無効化**（状態条件で書かなかったときは無効化しない）（採用）。修正後の再レビューは省略（ユーザー判断）。
- 実装は task_05d。
