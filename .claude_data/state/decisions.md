...
## 2026-09-17 (phase 19: フル表示ヘッダの幅をウィンドウ最小幅に含める)

### 【起票】暫定仕様 17 の確定（ユーザー 2026-09-17）

- 方式 = **案 A**（ウィンドウ最小幅 = max(メイン, ヘッダ)・配置は変えない）/ 固定対象 = フックの枠の切替ボタン 4 つ / ドロップダウン 18 → 12 は**フル・省略の両方**。
- 起票時 `deep-reviewer`（修正して採用）: メインの実測表が取得中状態で測られていた誤り（標準の不足 55 → 10px・起動時の拡幅 835 → 799px）を訂正 /
  ボタン幅は実測ループでなく `font.measure` の純関数・担当は各ボタンの制御側 / 文言定数は中立なモジュール / 両方が画面超過のときの縮小目標 / 既存テストの期待値変更を列挙。
- 確定前 `codex-adversarial-reviewer`（中 1・裏取り済）: ドラッグ後の最小幅が画面超過の縮小規則を通ると実表示より小さくなる → 縮小規則を通さない（採用）。
- §4 の判断: 起動時に自動で広がる = **受容** / 省略表示のボタン = **固定しない**（固定すると幅 270 で常時切れる実測）/
  旧保存値が最小幅未満 = **推奨（受容）ではなく「起動時に広げた幅で保存値を更新」**（条件: 保存値あり・切り詰め前の保存値 < 適用後の幅。切り詰めのみ・保存値なしは書かない）。

### 【task_05】二次レビューの採否（ユーザー 2026-09-17・推奨どおり）

- `codex-reviewer` = 指摘なし / `deep-reviewer` = 完了可。
- 採用 → task_06: 正本 `features.md:62,65,70` の §2-6・§3-2 との矛盾解消 / 新規 2 モジュールの `codebase_map.md` 記載 / 「保存値 > 画面幅でも最小幅未満なら書く」の注記。
- 採用 → task_05b: 上記境界の単体テスト。周知のみ: 大きいフォントで起動すると広げた幅が保存値に残る（仕様どおり）。
- 保留（refactor_check 候補）: 保存値検証の重複 / ドロップダウン幅定数の置き場所 / 2 モジュールの保存予約遅延。除外: 3 件。
