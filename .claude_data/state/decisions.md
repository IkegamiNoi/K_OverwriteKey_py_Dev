...
## 2026-09-16 (phase 17: 最小化中の grab 預かり)

### 【task_03 で検出した仕様の穴】§6-10 後半が現行設計では満たせない → **A案を採用**（ユーザー判断）

- **検出**: task_03 の受け入れテスト A9 が fail。verifier のトレースで機序を確定した。
  預かり中に新モーダルが開くと §3-2(7) により **`_custody_window` が新モーダルの `previous` へ
  移譲されクリアされる**。その新モーダルが**復元より前に破棄**されると、phase 14 の
  `<Destroy>` 復元は **`previous` が最小化中で `viewable`=0 のためスキップ**し、
  預かりも空のままになる → **復元後に grab 保持者がゼロ**（`grab_current()` が `None`）。
- **判断**: **採用 = A案（預かりへの差し戻し）**。`restore_grab` が復元をスキップした場合、
  **最小化中に限り** その `previous` を `_custody_window` へ戻す。
  最小化中かどうかを見るフラグを `modal.py` に 1 つ持つ（App の `<Unmap>` で立て `<Map>` で下ろす）。
  - **フラグ無しで無条件に戻さない**理由: 最小化していないときに非表示の窓が預かりへ入ると、
    **次の最小化で「既に預かり中」ガードに阻まれ本来の預かりが起きない**（元の欠陥が再発する）。
  - 差し戻すのは**預かりが空のときだけ**。
- **除外した案**: **B案（§6-10 後半を落として非モーダル化を受容）** = §6-8 が禁じている事象と
  実質同じものを別経路で許すことになり条項間の一貫性が崩れる。
  **C案（`<Map>` で預かり記録に依らず台帳の最内へ張り直す）** = 最小化と無関係な App の map でも
  発火し副作用が読みにくい（起票時に §3-3 で非推奨とした案 (b) 相当）。
- **反映**: 暫定仕様 15 を **v0.5** へ改訂（§3-2 に **(8)** 追加 / §6-10 に注記）。
  実装は**枝番タスク task_02b**（`modal.py` のみ）。

### 【task_03 の A7】テスト側の不具合（仕様判断ではない）

- `withdraw()` した transient 子は **App の `deiconify()` に追従して再表示される**（OS / Tk 側の挙動）ため、
  「記録窓が復元時も非表示」という前提をこの作り方では成立させられない。
  **`grab_set` が呼ばれないことの検証（本題）は pass している**。テストの作り方を変えて固定し直す。

### 【task_04 の二次レビュー H-1】v0.5 の差し戻しに残穴 → **最小修正を採用**（ユーザー判断 2026-09-16）

- **検出**: `deep-reviewer`（フェーズ横断）が実測で再現。**メインセッションでも裏取り済み**
  （`P exists/viewable: 1 1 | grab_current: None`）。
  最小化中に新モーダル N が開き（§3-2(7) で預かりが N の `previous` へ移譲）、
  **預かり窓 H と N の両方が復元前に破棄**されると、§3-2(8) は「生存しているが非表示」しか
  拾わないため差し戻しが起きず、`<Map>` は `_custody_window is None` で即 return して
  **台帳フォールバックへ到達しない** → 生存・表示中の外側モーダル P が非モーダルのまま復元される。
- **判断**: **採用 = 最小修正**（枝番 task_02c）。**`previous` が破棄済みでも差し戻す**
  （復元できなかった理由で区別しない）。`<Map>` の候補ループが受け皿になるため `<Map>` 側は無変更。
  暫定仕様 15 を **v0.6** へ。**除外した R-1（`_custody_window` を廃して `<Map>` で常に台帳最内へ）は
  C案の蒸し返しのため不採用**（判断は本ファイルの上節どおり）。
- **同時に閉じるテストの穴**（同レビューの M-3 / M-4・テストのみ）:
  **M-3** = `<Unmap>` 側の「解決不能なら預からない」「保持者が破棄済みなら預からない」
  「既に預かり中なら何もしない」が未固定（消しても落ちない）。
  **M-4** = `_app_minimized` がテストモジュール間でリーク（`setUp` で patch していない）。
- **文書だけで扱う指摘**（task_05 で正本・アーカイブへ）:
  **M-1** = 最小化で隠れない窓（`KeyboardWindow` は `transient` を持たない）は**預かり中に操作できる**
  （復元で grab は戻る。対策前より悪化はしない）→ **残存リスクとして明記**。
  **M-2** = `<Map>` のフォールバックは実装の方が条文より広い（記録窓が生存かつ非表示でも台帳最内へ）
  → **昇格文面を実装に合わせる**（実装は変更しない）。
  **L-2** = 最小化中に stdlib ダイアログが grab を持つと預かりが捨てられ、復元後に非モーダルになる
  → 「保証の範囲外」条項へ 1 行。
- **Codex レビューは利用上限で実行不能**。`agent_selection.md` の規定どおり
  **Claude 側（`deep-reviewer`）へ縮退**（報告のみ・許可不要）。
