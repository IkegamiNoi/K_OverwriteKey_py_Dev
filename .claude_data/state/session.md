...
## current
focus: **phase 36（config_service の分割と巨大関数の分割）task_01〜03 完了。次 = task_04（`save_runtime_data` の分割）。**
mode: implementing
...
result_files:
  ...
  - keyseq/application/config_service/{child_file_io.py, split_loading.py}
  - instructions/phase/36_config_service_split/{phase.md, tasks/task_01_*, tasks/task_02_*, tasks/task_03_*, tasks/task_04_*（起票のみ）}
verified:
  compile: clean
  tests: 674 ran OK（skipped 7）
  tests_ui: 576 ran OK
  smoke: pass
  review: reviewer 完了可（task_01〜03）

## next_action
- **task_04 を codex-implementer へ委任**: `instructions/phase/36_config_service_split/tasks/task_04_split_save_runtime_data.md`（起票済）→ verifier → reviewer。
  以降 task_05〜07（関数分割・タスク定義は着手時に起票）→ task_08（記録・`/refactor_check`）。
