...
### 【task_06】フェーズ完了判定の 2 本立てレビュー（2026-09-05）= **全件採用**（ユーザー確定）

対象は**正本反映の差分のみ**（実装差分は task_05 でレビュー済のため再レビューしない）。
**両者とも「修正要 / needs-attention」**で、指摘は**すべて暫定仕様 v0.5 → 正本の転記漏れ**。
**コード変更・仕様変更はゼロ**（内容は既にユーザー確定済の条項）。

- `deep-reviewer` = **修正要**（H1〜H13）/ `codex-adversarial-reviewer` = **needs-attention**（High 2 / Medium 1）。
  **2 件は両レビュアーが独立に同じ箇所を指摘**（H6 = Codex#3 の正規化副作用 / H7 = Codex#2 の競合窓）。
- **H1（High・最重要）**: 正本へ「保護対象は**提示**・件数・0 件警告から除外」と書いたが、
  暫定仕様の原文は「**「消えるパス」の一覧**・件数・判定から除外（表示するなら『保護のため残す』と別枠）」。
  **実装は別枠で提示している**ため正本の字面が実装と矛盾していた。
  → 放置すると「仕様書が正」で **v0.4 で潰した「表示と実行結果の乖離」が別の形で再発**する。
- **C1（High・Codex）**: `phase.md` が凍結を明記した直後に「**詳細は暫定仕様が正**」と書いており、
  凍結運用（条項を実装の根拠に引かない）と正面から矛盾 → **完了フェーズの経緯要約**へ書き換え、
  正を `data_schema.md` §5.8.1 / `features.md` §4.6 / `codebase_map.md` に限定した。
- **H2〜H10 = 転記漏れの補完**（すべて §5.8.1 へ追記）: 同一ファイルへ解決される子の**重複排除** /
  **判定名 4 種と SKIP 条件**（§5.8.4 の「分岐は判定名で行う」契約に対応）/
  **確認 UI は読み取り専用 1 枚・行ごとの選択なし・キャンセルで 1 ファイルも書かない** /
  **実行結果の通知**（既知の制約が通知の存在を前提にしていた）/ **残す参照元の正規化副作用**
  （strip・完全一致重複除去・非 str 除去は同時に起こる）/ **競合の一般条項**（読み直し〜書き込みの窓は
  対象外・版情報の照合は行わない）/ **未実体化の子と未保存削除の子は対象外** /
  **0 件警告に添える内容** / **「§5.1 の例外ではなく値の除去を行う唯一の経路」への言い換え**。
- **H13（参考）**: `idea_12` の前提が凍結済み暫定仕様を指していた → **正本 §5.8.1 の併記**へ変更。
  `codebase_map` の「このクラス自身はロジックを持たない」も 0 件分岐の実態に合わせて表現を修正。
- **`/refactor_check` 判定「不要」は deep-reviewer も規定どおりで妥当と確認**
  （M1 は AND 条件で非該当 / M6 は照合範囲的にも抑止条件的にも二重に非該当）。
  指摘は**完了報告の文面**が「M1〜M6 該当なし」と「M6 候補を抑止」で矛盾して見える点のみ（本節で書き分け済）。
- **得られた知見**: **暫定仕様を凍結するフェーズ末では、正本へ転記されなかった条項が
  そのまま失われる**。§7 の昇格表（対象ファイル単位）だけを追うと**条項単位の漏れを見落とす**ので、
