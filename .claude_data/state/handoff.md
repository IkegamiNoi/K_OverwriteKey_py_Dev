...
3. 主入力の確定設計 = [history/08_per_keymap_set_presets.md](../../instructions/history/08_per_keymap_set_presets.md)
   （**v0.10・全条項が確定済**。**§2 が確定事項の集約**。§3-2 解決順序 / §3-3 保存 / §3-4 UI が実装の規範。
   **版が多いので古い版の条項を引かないこと**）
...
## 現在の作業の 1 行サマリ
**v0.8 の実機目視は OK。その後のレビュー指摘を v0.9（【S】上書き確認 /【I2】OFF 開時の一覧確定）、実機目視 項目 17 の「既存を採る手段が無い」を v0.10（【S】を 3 択化）として確定し、task_07f・07g とも実装完了。残るは 4 項目の実機目視〔ユーザー作業〕→ task_08 正本反映**。
...
```
直近の実測（phase 09 task_07g 完了時・コミット `e5b0f12`）:
compile **clean** / tests **238** / tests_ui **223** / smoke **pass** /
manual **v0.8 分 16 項目は OK 済・4 項目（13 / 17 / 18 / 19）が未実施**。
**件数が減ったら退行を疑う**（**ただし tests の 242 → 238 は退行ではない**＝判定 API の戻り値変更に伴い
単体 5 本を 1 本へまとめた分。task_07g）。実行後に worktree ルートへ `user/` が生成されていないことも確認する。
...
- **【ユーザー作業・これが残り全部】追加 4 項目の実機目視**。観点は
  ...
  **v0.8 分の項目 1〜16 は 2026-08-15 に OK 済**なので、残るのは次の 4 つ:
  - **17**（個別 ON のセット A がある状態で**別のセットを A の名前で別名保存**→ そのセットを **ON にして OK**
    → **上書き確認が 3 択で出る**〔v0.10〕。**キャンセル でファイル・一覧・フラグ・dirty が全て不変、
    ダイアログも閉じない** / **既存を読み込む で一覧が既存の内容へ差し替わり、続けて OK すると
    確認なしで保存されて既存の内容が保たれる** / **上書きする で置き換わる**）
  ...
  - 目視で不具合が出たら **task_07 内で是正**（最小差分 + `reviewer` 再実施）。
    **仕様変更を伴うなら枝番タスクへ切り出す**（07b〜07g と同じ流儀）。
- 次に **task_08 = 正本反映（最終）**: `data_schema.md` §5.10 改訂 + **§5.10.4（v0.9）** + §5.5 / §5.4 / §5.8.8 / §5.1 +
  ...
  - **v0.5→v0.6 の反転（【O3】）/ v0.7 の【O4】/ v0.8 の 3 改訂 / v0.9 の【S】【I2】/ v0.10 の 3 択化 /
    dirty 規則 / task_05b が task_05c で置き換わった経緯**を decisions_archive へ集約する。
- 各タスクの流れ: タスク定義起票 → codex-implementer へ委任 → **verifier で実測** → reviewer → コミット。

## 現フェーズ（phase 09 = keymap_set ごとの個別プリセット）の要点

**確定設計は暫定仕様 08（v0.10）が正**。**実装は task_01〜07g まで完了**（残りは実機目視 → task_08）。
...
- **【v0.10】【S】の確認は 3 択**（**上書きする / 既存を読み込む / キャンセル**）。2 択では
  **既存ファイルの内容を採る手段が無かった**（記録パスが空だとトグル読み直しが空振りし、
  **パスが記録されるのは保存成功後**のため到達不能）。
  - **adopt = 一覧と `_loaded_temp` の両方を実体の内容へ差し替えるだけ**。**保存しない・閉じない・
    追加の破棄確認も出さない**。続けて OK すると内容一致で確認が出ず、既存の内容が保たれる。
  - **cancel と adopt はどちらも `save_hotkey_presets` が `False`**（`data`・dirty・ファイル不変）。
  - **破損で読めないときは「既存を読み込む」を出さず 2 択**（本文も「読み込めません」に出し分ける）。
  - 3 択は **`messagebox` ではない自作 `Toplevel`**（`hotkey_presets_io`）。**判定 API は
    `conflict` + `existing` を同時に返す**（同じファイルを 2 回読まないため。旧 bool API は残さない）。
...
- **blockers: task_07 の完了に実機目視（ユーザー作業）が必要**。それまでフェーズ完了判定は出せない。
  **v0.8 分 16 項目は OK 済・v0.9 / v0.10 の実装（task_07f・07g）も入った**ので、
  残るは**4 項目（13 / 17 / 18 / 19。17 は 3 択版）のみ**。
- **【既知・触らない】ネストしたモーダルを閉じると親の grab が復元されない**
  （モーダル中のモーダル。**既存の「追加」「編集」も同じ**）。**[idea_10] として分離済**で、
  **新しいダイアログにも復元処理を書かない**（挙動を揃えるため）。
  「マネージャを開いたままメインを触れる」のはこの既知事象。
...
- 未着手/保留 idea: idea_07（参照元の掃除・着手可）/ **idea_10（ネストしたモーダルの grab 復元・
  phase 09 の敵対的レビューから分離）** / idea_03（hotkey 保存正規化・低）/
