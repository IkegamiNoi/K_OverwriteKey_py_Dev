...
**次にやること = 計画09 = `/spec_split` の分割計画を提示して承認を得る**
（`data_schema.md` 907 行の §5.8 / §5.10 を INDEX 分割。ミニ計画としての実施はユーザー確定済）。
その後、次フェーズの起票（`/phase_start`・採番 12）。
...
- **【最優先】計画09 = `/spec_split` の分割計画を提示し、ユーザー承認を得てから実施する**。
  対象 = `data_schema.md`（**907 行**）の **§5.8（約 240 行）/ §5.10（約 220 行）**。
  **節番号・見出しは変更しない**・本文は **verbatim 外出し**で**分割前コミットとのバイト一致**を確認・
  親 1 つ分 = 1 コミット・**CRLF に注意**（手順は `.claude/commands/spec_split.md`）。
  - 済: **提案書 08 は「計画08」として実施・完了**（2026-09-08）/
    **候補側リンクを無音で落とす挙動は「契約として明記のまま」でユーザー確定**（正本 §5.8.9 の制約 12）。
