...
4. **このフェーズの設計の正は暫定仕様 [05_child_file_save_dialog.md](../../instructions/history/05_child_file_save_dialog.md)**
   ...
   タスク定義は `instructions/phase/06_child_file_save_dialog/tasks/`（task_01〜06b は起票・完了済）。
...
## 現在の作業の 1 行サマリ
**Phase β task_06 + task_06b（統合退行とレビュー指摘 A〜F の修正）の自動検証まで完了。残るは実機目視（ユーザー実施）→ task_07（正本反映）**。
...
```
直近の実測（task_06b 完了時・verifier）: compile **clean** / tests **131** / tests_ui **107** / smoke **pass**。

## 次アクション（session.md.next_action より）
- **実機目視をユーザーへ依頼する**（残る唯一の未完了項目）。起動は worktree ルートで
  `../../../.venv/Scripts/python.exe main.py`。確認項目は task_06 定義「対象範囲 4」の 9 項目 +
  deep-reviewer 推奨の 6 項目（読込直後の個別「トリガー一覧を保存」で保存先を聞かれないか /
  その後の一括保存がどちらのパスへ書くか / 別名保存ダイアログのキャンセルで一覧へ戻るか /
  再表示で選択がリセットされるのを許容できるか / 新規子を「保存しない」にして再読込で消えないか /
  ×ボタンでキャンセル扱いになるか）。結果は `integration_result.md` §3 へ記録する。
...
- **依存関係の扱い（ユーザー確定 2026-07-29）**: 親 keymap_set は**問わない**（「保存」操作自体が明示のため
  無確認保存）/ **trigger_set は問う** → **OK 押下時に確認ダイアログ**（保存 / 別名保存 / 選び直す）。
...
- **壊しやすい不変条件**（レビューで実際に破れていた箇所。変更時は必ず確認する）:
  ...
  ③ **提示した保存先と実際に書く先を一致させる**（trigger_set の保存先が変わったら再解決 + 一覧再表示）
...
- **主要モジュール**: `keyseq/application/save_plan.py`（`SavePlan` / `ChildSaveEntry` / `SavePlanError`）/
  ...
  `find_dependency_blocked_sequences` ・ `read_parent_refs`）/ `config_io/child_save_rows.py`（行モデル）/
  `child_save_plan.py`（選択 → 計画の純変換）/ `child_save_dialog.py`（UI）/
  `keymap_set_io._collect_child_save_plan`（再解決ループ）。
- **暫定仕様に対して足した判断（task_07 で正本へ明記）**: 上記「次アクション」の task_07 リスト参照。

## 注意事項・blockers
- **blockers: なし**。
- **【Codex 運用】フォワーダが最終出力を返さないまま完了通知だけ来る / タイムアウトしてジョブだけ走り続ける**
  ことがある。後者は `node "<plugin>/scripts/codex-companion.mjs" status --all` で job id を確認し、
  `plugins/data/codex-inline/state/<worktree>-<hash>/jobs/<job>.json` の `status` が `running` でなくなるまで待つ
  （バックグラウンドの until ループが有効）。**Codex 申告のテスト結果は信用せず必ず verifier で実測**。
  詳細は `instructions/common/rules_detail/codex_operations.md`。
...
- **【傾向】テストの期待値誤りが混ざる**（Codex 実装物で実測済み 2 件: `slugify_file_stem` は大文字小文字を
  変換しない / `source_path` は config_root 相対なので読み出し時に root と結合が要る）。
...
- **β で保留にしたレビュー指摘**（task_07 or 別 idea で扱う）: sequence の共有判定の「現在の上位」が
  計画後の trigger_set 保存先である点 / 事前検証で `os.makedirs` の副作用がある点 / 一覧再表示で選択が
  全リセットされる点 / `_ask_save_as_path` が `choose_save_path_with_collision` を使っていない点 /
  一覧の縦スクロール未実装。
