...
def build_menu_bar(app):
    ...
    file_menu.add_command(label="新規作成", command=app.config_io.new_config, accelerator="Ctrl+N")
    file_menu.add_separator()
    file_menu.add_command(label="保存", command=app.config_io.save_keymap_set, accelerator="Ctrl+S")
    file_menu.add_command(label="別名で保存…", command=app.config_io.save_as, accelerator="Ctrl+Shift+S")
    file_menu.add_command(label="読込（構成セット）…", command=app.config_io.load_keymap_set_from, accelerator="Ctrl+O")
    file_menu.add_separator()
    file_menu.add_command(label="Import...", command=app.config_io.import_config)
    file_menu.add_command(label="Export...", command=app.config_io.export_config)
    file_menu.add_separator()
    file_menu.add_command(label="起動時に読む構成セットを指定…", command=app.config_io.set_startup_keymap_set)
    file_menu.add_command(label="例を復元", command=app.config_io.restore_default)
    file_menu.add_separator()
