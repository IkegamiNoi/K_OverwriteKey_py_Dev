...
class ConfigIoController:
    ...
    def new_config(self):
        ...
        self._app.keymap_set_path = self._app._preferred_keymap_set_path()
    ...
    def save_as(self, *, show_success_dialog: bool = True) -> bool:
        suggested_path = self._app._suggest_keymap_set_dialog_path()
        path = filedialog.asksaveasfilename(
            title="別名で保存（keymap_set）",
            initialdir=self._app._suggest_keymap_set_dialog_dir(),
    ...
    def save_keymap_set_to(self, path: str, *, flash_message: str, show_success_dialog: bool) -> bool:
        try:
            save_path = self._app._normalize_keymap_set_save_path(path)
            ...
            self._app.startup_path = self._app._preferred_startup_path()
            self._app._startup_settings = startup_payload
            self._app._clear_individual_dirty_flags()
            self._app._set_dirty(False)
    ...
    def set_startup_keymap_set(self):
        ...
        self._app.keymap_set_path = path
        self.write_startup({"keymap_set_path": self._app._to_config_relative_or_absolute(path), "prompt_if_missing": True})
        self.apply_loaded_data_to_ui()
        self._app.state.reset_indices()
        self._app._refresh_triggers()
        self._app._refresh_actions()
    ...
    def apply_loaded_data_to_ui(self):
        self._app._trigger_set_source_path = ""
        self._app._trigger_set_imported = False
        self._app._trigger_set_dirty = False
        self._app._sync_control_vars_from_data()
        self._app._clear_individual_dirty_flags()
        self._app._set_dirty(False)

    def choose_split_base_dir_for_keymap_set(self, save_path: str) -> str:
        if self._app._is_within_config_root(save_path):
            return ""
    ...
    def load_startup_and_config(self):
        ...
        if stored_keymap_set_path:
            resolved_keymap_set_path = self._app._resolve_keymap_set_path(stored_keymap_set_path)
    ...
    def ask_link_label_to_filename(self, *, title: str, path: str) -> bool:
        ...
        ttk.Label(frame, text=f"保存名: {self._app._filename_stem(path)}").pack(anchor="w")
    ...
    def save_selected_keymap(self) -> bool:
        ...
        if not source_path:
            ...
            suggested = self._app._suggest_json_path(self._app._preferred_keymaps_dir(), label, "keymap")
    ...
    def save_selected_keymap_as(self) -> bool:
        ...
        source_path = str(keymap.get(self._app.config_service.INTERNAL_KEYMAP_SOURCE_PATH) or "").strip()
        label = str(keymap.get("label") or keymap.get("id") or "keymap").strip()
        suggested = self._app._suggest_json_path(
            self._app._json_dialog_initial_dir(self._app._preferred_keymaps_dir(), source_path),
            label,
            "keymap",
        )
        path = filedialog.asksaveasfilename(
        ...
        try:
            if self.ask_link_label_to_filename(title="キーマップ名の連動", path=path):
                keymap["label"] = self._app._filename_stem(path)
        except RuntimeError:
            return False
    ...
    def load_keymap_file(self) -> None:
        path = filedialog.askopenfilename(
            title="キーマップを読込",
            initialdir=self._app._json_dialog_initial_dir(self._app._preferred_keymaps_dir()),
            filetypes=[("JSON", "*.json"), ("All", "*.*")],
        )
        if not path:
            return
    ...
    def save_trigger_set_file(self) -> bool:
        ...
        if path and self._app._trigger_set_imported and self._app._trigger_set_dirty:
            if messagebox.askyesno("保存", "読込で持ってきたトリガー一覧です。\n別名で保存しますか？"):
                return self.save_trigger_set_file_as()
        if not path:
            suggested = self._app._suggest_json_path(self._app._preferred_trigger_sets_dir(), self._app._keymap_set_file_stem(), "trigger_set")
    ...
    def save_trigger_set_file_as(self) -> bool:
        ...
        suggested = self._app._suggest_json_path(
            self._app._json_dialog_initial_dir(self._app._preferred_trigger_sets_dir(), source_path),
            self._app._filename_stem(source_path) or self._app._keymap_set_file_stem(),
            "trigger_set",
        )
        path = filedialog.asksaveasfilename(
            title="トリガー一覧を別名で保存",
    ...
    def save_trigger_set_to_path(self, path: str) -> bool:
        try:
            ...
            self._app._trigger_set_source_path = path
            self._app._trigger_set_imported = False
            self._app._trigger_set_dirty = False
            self._app._refresh_triggers()
            self._app._refresh_actions()
    ...
    def load_trigger_set_file(self) -> None:
        ...
        path = filedialog.askopenfilename(
            title="トリガー一覧を読込",
            initialdir=self._app._json_dialog_initial_dir(self._app._preferred_trigger_sets_dir()),
            filetypes=[("JSON", "*.json"), ("All", "*.*")],
        )
        if not path:
            return
        try:
            ...
            self._app._trigger_set_source_path = path
            self._app._trigger_set_imported = True
            self._app._trigger_set_dirty = False
            self._app.state.reset_indices()
            self._app._selected_trigger_idx = 0
            self._app._refresh_triggers()
    ...
    def save_selected_sequence(self) -> bool:
        ...
        if not source_path:
            ...
            suggested = self._app._suggest_json_path(self._app._preferred_sequences_dir(), label, "sequence")
    ...
    def save_selected_sequence_as(self) -> bool:
        ...
        label = str(trigger.get("label") or trigger.get("key") or "sequence").strip()
        suggested = self._app._suggest_json_path(
            self._app._json_dialog_initial_dir(self._app._preferred_sequences_dir(), source_path),
            label,
            "sequence",
        )
        path = filedialog.asksaveasfilename(
        ...
        try:
            if self.ask_link_label_to_filename(title="出力シーケンス名の連動", path=path):
                trigger["label"] = self._app._filename_stem(path)
        except RuntimeError:
            return False
    ...
    def load_sequence_file(self) -> None:
        ...
        path = filedialog.askopenfilename(
            title="出力シーケンスを読込",
            initialdir=self._app._json_dialog_initial_dir(self._app._preferred_sequences_dir()),
            filetypes=[("JSON", "*.json"), ("All", "*.*")],
        )
        if not path:
            return
