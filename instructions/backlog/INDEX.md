...
| idea_34 | [idea_34_focus_restore_after_minimize.md](idea_34_focus_restore_after_minimize.md) | モーダルを開いたまま最小化 → 復元すると、grab はダイアログへ戻るがキーボードフォーカスはメイン窓に残り、クリックしない限り Escape が効かない。復元時にフォーカスも最内の表示中モーダルへ戻したい。 | **着手**（→ 暫定仕様 [23](../history/23_focus_restore_after_minimize.md) / phase [29_focus_restore_after_minimize](../phase/29_focus_restore_after_minimize/phase.md)）|
