...
| idea_25 | [idea_25_path_field_type_normalization.md](idea_25_path_field_type_normalization.md) | keymap_set / 外部レイアウト登録の**パス系フィールド**（`path` / `switch_key` / `trigger_set_path` 等）に `str()` 強制が残っており、非文字列を渡すと Python の repr 文字列がパスとして扱われる。例外にはならず「存在しないパス」として無視されるため実害は小さい。正本 §5.5 / §5.7 に型規定が無い。 | 未着手（検討段階・2026-09-19・phase 24 task_01 の全走査で発見・スコープ外として分離・優先度低）|
