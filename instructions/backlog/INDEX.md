...
| idea_14 | [idea_14_config_service_public_surface.md](idea_14_config_service_public_surface.md) | presentation が `config_service` の内部モジュールから定数・結果型を直 import している点を、**公開面モジュールを 1 つ新設して定義ごと集約**する（**phase 10 分も同時に変換**）。挙動不変。 | 未着手（検討段階・**優先度低〜中**・2026-09-08 起票。phase 11 task_07 の `deep-reviewer` 指摘 B-2 から分離。**ユーザー判断で当面は条項側に例外を明記**〔`architecture.md` §3.2〕し実装は据え置き。**着手トリガー**: 実装モジュールの再分割・改名 / 直 import のさらなる増加 / `__init__.py` の分割着手）|
