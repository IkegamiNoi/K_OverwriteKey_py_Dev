...
| idea_14 | [idea_14_config_service_public_surface.md](idea_14_config_service_public_surface.md) | presentation が `config_service` の内部モジュールから定数・結果型を直 import している点を、**公開面モジュールを 1 つ新設して定義ごと集約**する（**phase 10 分も同時に変換**）。挙動不変。 | **着手**（→ phase [12_config_service_public_surface](../phase/12_config_service_public_surface/phase.md) / 暫定仕様 [11_config_service_public_surface](../history/11_config_service_public_surface.md)・2026-09-08 起票）。phase 11 task_07 の `deep-reviewer` 指摘 B-2 から分離。**挙動不変のリファクタ**で、`architecture.md` §3.2 の当面の例外を実装ごと解消する。**phase 10 由来の分も同時に変換する**（2 形式を併存させない）|
