...
| idea_17 | [idea_17_action_dialog_preset_manager_parent.md](idea_17_action_dialog_preset_manager_parent.md) | `ActionDialog` からプリセット編集を開くとき親として App を渡しており、`transient` 親が実体と食い違う（`action_dialog.py:339`）。**論点は z 順のみで grab の復元先には影響しない**。付け替えるには「親ウィンドウ」と「App 参照」の引数分離が要る。 | **着手**（→ phase 16 [dialog_transient_parent](../phase/16_dialog_transient_parent/phase.md) / 暫定仕様 [14](../history/14_dialog_parent_and_app_separation.md)・2026-09-13。**実機で症状を確認済**〔アクション編集を掴んで動かすとプリセット編集より前に出る〕。**前面維持の指定だけを直し、所有関係は動かさない**〔動かすと破棄が連鎖する〕。idea 本文の「引数で分離する必要がある」は所有関係を動かす前提の記述で、**採らない**）|
