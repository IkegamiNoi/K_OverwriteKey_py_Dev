...
| idea_35 | [idea_35_unknown_action_type_handling.md](idea_35_unknown_action_type_handling.md) | 空 / 未知のアクション `type` は現在 `value` を文字入力する（`action_executor.py:64`）。何も送らずエラー通知に変えたい。空 `type` の解釈が executor（text）とダイアログ（hotkey・`action_dialog.py:115`）で食い違う点も揃える。 | **着手**（→ 暫定仕様 [24](../history/24_unknown_action_type_handling.md)〔v0.5・確定済〕/ phase [31_unknown_action_type_handling](../phase/31_unknown_action_type_handling/phase.md)・2026-09-24。案 B〔何も送らずエラー通知・シーケンスは止める〕）|
