...
| idea_19 | [idea_19_minimize_restore_with_child_grab.md](idea_19_minimize_restore_with_child_grab.md) | 子ダイアログが grab を持つ状態で Win+D（全最小化）すると**アプリを再表示できず操作不能になる**。最小化・復元に関する結線がアプリ側に無く、復帰が OS 任せであることが背景。原因は未確定で、着手時はまずウィンドウ状態の実測が要る。 | **着手**（→ phase [17_minimize_grab_custody](../phase/17_minimize_grab_custody/phase.md) / 暫定仕様 [15](../history/15_minimize_grab_custody.md)・2026-09-16 起票）。**原因は実測で確定**〔シェルの復元要求が `withdrawn` の grab 保持者へ向かい、App には `<Map>` が来ない〕。**phase 16 由来ではない**〔変更していないネスト経路でも再現・2026-09-15〕|
