...
| idea_26 | [idea_26_dialog_keyboard_focus.md](idea_26_dialog_keyboard_focus.md) | Escape を bind しているのにキーボードフォーカスを取らないダイアログ（`orphan_sweep` / `quarantine_manage` / `reference_cleanup`）があり、実使用で Escape が効かない。既存テストは `focus_force()` 後に Escape を送るため検出できない。 | **着手**（→ [phase 28](../phase/28_dialog_keyboard_focus/phase.md) / [暫定仕様 22](../history/22_dialog_keyboard_focus.md)。**案 B=`grab_modal` へ集約**で確定〔実装方式は明示引数〕。群 C の 5 経路への Escape 追加と正本への 2 条項追記まで含む）|
...
| idea_33 | [idea_33_hook_resume_after_idle_flaky_test.md](idea_33_hook_resume_after_idle_flaky_test.md) | ダイアログ破棄後のフック再開が `after(0)` 予約のため、CPU 負荷下で 1 回の `app.update()` に拾われず `get_hook_pause_count()` が 1 のまま残る。`setUp` のドレイン検査で同クラスの後続テストが連鎖して落ちる（1 fail が 7 件の赤に）。 | 未着手（検討段階・**テストのみ・production 不変**・優先度中・2026-09-23 起票。phase 28 task_05 の §8-7 判定から分離〔ユーザー判断 = §8-7 は Escape family に限定〕。**idea_18 とは別 family**〔Escape を使わない経路でも起きる〕。**phase 28 由来でないことを A/B 実測で確認済**。推奨は案 A〔フック再開を期限つきで待つヘルパ〕）|
| idea_34 | [idea_34_focus_restore_after_minimize.md](idea_34_focus_restore_after_minimize.md) | モーダルを開いたまま最小化 → 復元すると、grab はダイアログへ戻るがキーボードフォーカスはメイン窓に残り、クリックしない限り Escape が効かない。復元時にフォーカスも最内の表示中モーダルへ戻したい。 | 未着手（検討段階・**仕様変更あり**〔`features.md` §4.6〕・2026-09-23 起票。phase 28 task_05 の実機目視 C②〔暫定仕様 22 §8-10〕で再現。`modal.py` の `return_custody` が `grab_set` のみで `focus_set` しないのが原因）|
