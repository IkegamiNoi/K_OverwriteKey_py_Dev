...
| idea_32 | [idea_32_grab_modal_static_check_discovery.md](idea_32_grab_modal_static_check_discovery.md) | `grab_modal` の静的検査（`tests_ui/test_nested_modal_grab.py:260`）が対象を dialogs 10 クラス + config_io 3 ファイルのハードコード列挙で持つため、新規ダイアログを守らない。発見ベースへ寄せたいが件数アサートが担う「呼び出しが消えた」検出を失う。 | **着手**（→ [phase 33](../phase/33_grab_modal_static_check_discovery/phase.md)・2026-09-24・直接改訂モード。案 A'〔`Toplevel` 継承で発見 + 件数据え置き〕・phase 15 側は対象外）|
