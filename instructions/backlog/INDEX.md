...
| idea_12 | [idea_12_orphan_child_file_sweep.md](idea_12_orphan_child_file_sweep.md) | `config` 配下を全走査し、どの構成セットからも参照されない子ファイル（孤児）を検出する棚卸し機能。**逆方向検査（上位 → 子の索引走査）**の新設を伴う。削除まで行うかは着手時に判断（不可逆・全網羅不可のため隔離案あり）。 | **着手**（→ phase [11_orphan_child_file_sweep](../phase/11_orphan_child_file_sweep/phase.md) / 暫定仕様 [10_orphan_child_file_sweep](../history/10_orphan_child_file_sweep.md)・2026-09-06 起票）。前提 = phase 10 の完了（**充足・2026-09-05**）。**ユーザー方針「全検査と現在のセットのみを段階的に両方作る」の後半**。phase 10（暫定仕様 09）の検査・除去のコアを土台に載せる |
