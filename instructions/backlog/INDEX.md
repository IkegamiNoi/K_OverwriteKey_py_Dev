...
| idea_20 | [idea_20_full_view_resizable_panes.md](idea_20_full_view_resizable_panes.md) | フル表示のメイン領域で、アプリ幅の変更で伸縮するのをトリガー一覧にし、境界線ドラッグでキーマップ管理・出力シーケンスの幅を変えられるようにする。変えた幅は config.json に保存し再起動後も復元する。 | **着手**（→ 暫定仕様 [16](../history/16_full_view_resizable_panes.md)〔v0.3・ユーザー確定済〕/ phase [18_full_view_resizable_panes](../phase/18_full_view_resizable_panes/phase.md)・2026-09-16 起票）|
| idea_21 | [idea_21_full_view_header_width.md](idea_21_full_view_header_width.md) | フル表示のヘッダ（フック / 表示 / ファイル操作）の幅をウィンドウ最小幅で考慮する（ファイル操作の枠の中身が切れる）。文言が変わるボタンは最大サイズで固定し、キーボード選択のドロップダウンを 2/3 の幅にする。 | 未着手（検討段階・phase 18 task_05 の実機目視）|
| idea_22 | [idea_22_full_view_min_height.md](idea_22_full_view_min_height.md) | フル表示のウィンドウに縦方向の最小サイズを設ける（目安 = 各一覧の既定の高さの半分）。 | 未着手（検討段階・phase 18 task_05 の実機目視）|
