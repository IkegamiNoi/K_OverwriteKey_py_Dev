...
| idea_24 | [idea_24_sequence_payload_action_normalization.md](idea_24_sequence_payload_action_normalization.md) | 個別 sequence JSON の単体読込が `actions` の正規化（dict 以外の除去・`label` 整形）を通らず、非 dict 要素がそのまま runtime へ載る。正本 `data_schema.md` §5.11 では【実装未追従】として明記済（規定が正）。 | 着手（2026-09-19 → [phase 23](../phase/23_sequence_payload_action_normalization/phase.md)・案 A で実装追従）|
