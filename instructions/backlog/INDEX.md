...
| idea_07 | [idea_07_reference_link_cleanup.md](idea_07_reference_link_cleanup.md) | 子ファイル（keymap/trigger_set/sequence）に記録した参照元（上位ファイル）の実在を一括確認し、参照できないものをリストから除去する掃除機能。menu_bar から起動。 | **着手**（→ [phase 10](../phase/10_reference_link_cleanup/phase.md) / [暫定仕様 09](../history/09_reference_link_cleanup.md)・2026-08-16 起票）。β の参照元記録（`_parent_refs`・best-effort 更新）の陳腐化を回収する保守機能。**回収対象**: 上位の移動 / 削除で残る孤児 trigger_set と陳腐化した `_parent_refs`（β では「オーナー不在の孤児 / 前バージョンのファイル」への露出を許容と確定済み・正本 `data_schema.md` §5.8.1）|
...
| idea_10 | [idea_10_nested_modal_grab_restore.md](idea_10_nested_modal_grab_restore.md) | モーダルの中からモーダルを開いて閉じると親の grab が戻らず、親ダイアログを開いたままメインウィンドウを操作できてしまう（`PresetManagerDialog` の追加・編集など）。`wait_window()` 後に親が grab を取り直す作法を棚卸しして揃える。 | 未着手（検討段階・phase 09 task_07g の暫定仕様 v0.10 敵対的レビュー High 2 から分離。**task_07g では除外**とユーザー判断）|
...
| idea_12 | [idea_12_orphan_child_file_sweep.md](idea_12_orphan_child_file_sweep.md) | `config` 配下を全走査し、どの構成セットからも参照されない子ファイル（孤児）を検出する棚卸し機能。**逆方向検査（上位 → 子の索引走査）**の新設を伴う。削除まで行うかは着手時に判断（不可逆・全網羅不可のため隔離案あり）。 | 未着手（検討段階・**前提 = phase 10 の完了**・2026-08-16 起票）。**ユーザー方針「全検査と現在のセットのみを段階的に両方作る」の後半**。phase 10（暫定仕様 09）の検査・除去のコアを土台に載せる |
