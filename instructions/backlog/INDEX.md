...
| idea_15 | [idea_15_contracts_boundary_ast_coverage.md](idea_15_contracts_boundary_ast_coverage.md) | 公開面の逆戻り防止テスト（`tests/test_config_service_contracts.py`）の AST 検査に、**静的な記述のまま素通りする 3 経路**（完全修飾のドット参照 / エイリアス束縛経由 / `from keyseq import application` 経由）がある。`ast.Attribute` の完全修飾名解決と `asname` 追跡へ拡張する。**テストのみ・仕様変更なし**。 | **着手**（→ phase [13_contracts_boundary_ast_coverage](../phase/13_contracts_boundary_ast_coverage/phase.md)・2026-09-08 起票・**直接改訂モード**〔暫定仕様なし〕）。phase 12 のフェーズ完了レビューで `deep-reviewer` M-3 と `codex-adversarial-reviewer` が独立に指摘。**現在の presentation に該当参照は 0 件**で、将来の逆戻りを取り逃がすだけの欠落。**テストのみ・プロダクション不変・仕様変更なし**（案 A = 検査の強化を採用）|
