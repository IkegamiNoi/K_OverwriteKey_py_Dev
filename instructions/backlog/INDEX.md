...
| idea_27 | [idea_27_mouse_click_button_type_coercion.md](idea_27_mouse_click_button_type_coercion.md) | `mouse_click` の `button` が非文字列だと `action_executor.py:119` の `.strip()` で `AttributeError`。x / y と違い `try` の外にある。正本 §5.11.2 は「非文字列は未定義」と明記しているため直すなら仕様改訂が先。 | **着手**（→ [phase 30](../phase/30_action_and_internal_key_type_coercion/phase.md)・idea_28 と統合・2026-09-23。案 A + `type` も含め読込時正規化で対応）|
| idea_28 | [idea_28_runtime_internal_key_type_coercion.md](idea_28_runtime_internal_key_type_coercion.md) | runtime 専用の内部キー（`_keymap_source_path` 等・§5.8.2）が §5.1 の型規則に未追従で、`save_path_resolution.py:127` の `str(...)` を経て保存先パス候補に repr が混入し得る。 | **着手**（→ [phase 30](../phase/30_action_and_internal_key_type_coercion/phase.md)・idea_27 と統合・2026-09-23。入口〔`ensure_config_compatibility`〕で `coerce_label`）|
...
| idea_33 | [idea_33_hook_resume_after_idle_flaky_test.md](idea_33_hook_resume_after_idle_flaky_test.md) | ダイアログ破棄後のフック再開が `after(0)` 予約のため、CPU 負荷下で 1 回の `app.update()` に拾われず `get_hook_pause_count()` が 1 のまま残る。`setUp` のドレイン検査で同クラスの後続テストが連鎖して落ちる（1 fail が 7 件の赤に）。 | 未着手（検討段階・**テストのみ・production 不変**・優先度中・2026-09-23 起票。phase 28 task_05 の §8-7 判定から分離〔ユーザー判断 = §8-7 は Escape family に限定〕。**idea_18 とは別 family**〔Escape を使わない経路でも起きる〕。**task_05c 由来でないことは A/B で確認済・task_01 は原因候補に残る**〔2026-09-23 補足〕。推奨は案 A〔フック再開を期限つきで待つヘルパ〕）|
| idea_35 | [idea_35_unknown_action_type_handling.md](idea_35_unknown_action_type_handling.md) | 空 / 未知のアクション `type` は現在 `value` を文字入力する（`action_executor.py:64`）。何も送らずエラー通知に変えたい。空 `type` の解釈が executor（text）とダイアログ（hotkey・`action_dialog.py:115`）で食い違う点も揃える。 | 未着手（検討段階・2026-09-23 起票。phase 30 起票時に B 案として分離〔ユーザー判断 = phase 30 は A 案で現行挙動を §5.11 に明文化〕。**層跨ぎ・挙動変更のため着手時は暫定仕様先行モード**）|
