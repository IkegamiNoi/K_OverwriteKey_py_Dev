...
| idea_16 | [idea_16_wm_close_skips_destroy_override.md](idea_16_wm_close_skips_destroy_override.md) | ダイアログを × で閉じると Python の `destroy()` override が呼ばれず `resume_hook_after_dialog()` が飛ぶ。フック停止カウンタがずれたまま残り、以後ダイアログを開いてもフックが自動停止しなくなる（編集中のキー入力が誤爆し得る）。該当は `protocol("WM_DELETE_WINDOW")` 未登録の 5 クラス。 | **着手**（→ 暫定仕様 [13](../history/13_dialog_teardown_on_close.md)・2026-09-12。phase 14 task_04 の起票前調査で実測確認。**grab 復元は `<Destroy>` 結線のため × でも働く**＝ phase 14 の成果には影響しない独立不具合）|
| idea_17 | [idea_17_action_dialog_preset_manager_parent.md](idea_17_action_dialog_preset_manager_parent.md) | `ActionDialog` からプリセット編集を開くとき親として App を渡しており、`transient` 親が実体と食い違う（`action_dialog.py:341`）。**論点は z 順のみで grab の復元先には影響しない**。付け替えるには「親ウィンドウ」と「App 参照」の引数分離が要る。 | 未着手（検討段階・**優先度低**・2026-09-12 起票。phase 14 の受け入れ条件 15 / 暫定仕様 12 §6-5 による分離。**v0.1 の「付け替えないと復元先が曖昧」は誤りとして撤回済み**）|
