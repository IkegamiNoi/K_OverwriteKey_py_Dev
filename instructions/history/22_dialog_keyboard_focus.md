...
> 状態: **未凍結・v0.4・ユーザー確定済・実装着手可**。本書がこのフェーズの確定設計（フェーズ中は正本を直接改訂しない）。
...
6. Escape 依存テスト 4 箇所について、**外す / 残すの判定と理由を完了報告に列挙**した上で
   §6 のヘルパ経由になっている（§5・§6）。
7. **負荷下での再現確認**: idea_18 の実測条件（busy loop 4 本の負荷下で対象クラスを 6 回）で
   fail が出ないこと。加えて `tests_ui` 一括を連続 3 回 green（§6）。
...
11. 正本 `features.md` に §4 の 2 条項が入り、`codebase_map.md` の `modal.py` 節が追従
    （件数訂正を含む。フェーズ末・§10）。
...
- `instructions/common/spec_detail/features.md`「モーダルダイアログの作法」へ §4 の **2 条項**
  （フォーカス + Escape）を追記。
