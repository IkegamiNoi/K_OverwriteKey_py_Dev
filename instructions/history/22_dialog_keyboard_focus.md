...
> **v0.7 = フェーズ完了判定前レビューの反映（ユーザー確定 2026-09-23）**。①**L3**: 記録中・取得中に Esc を
> **押しっぱなし**にすると、キーリピートの 2 発目以降が通常時の Escape として扱われ**ダイアログが閉じる**（task_05c で入った退行）。
> **停止に使った Esc を離すまでは Escape で閉じない**規範を追加（§2.3・§3.6.1・§8-14）。②**H1**: §4 の Escape 条項の
> 「保存せずに閉じる（キャンセル相当）」が正本の既存条項（孤児ファイルの棚卸しは閉じても一覧の編集が保存される）と
> 矛盾 → 「**× と同じ結果で閉じる**」へ（動作不変）。③§4 の初期フォーカス条項へ §3.1 の**保証時点**を移す。
> ④**M2**: 群 A の「閉じない」検査の送り方を `send_escape` と同じフォーカス確保へ揃える（§8-14）。
...
### §2.3 追加の確定（ユーザー 2026-09-23・4 回目。フェーズ完了判定前レビューの決着）

11. **記録中・取得中の Esc を押しっぱなしにしてもダイアログを閉じない**（L3。ユーザーが実機で
    「停止 → 閉じる」を確認）。**停止に使った Esc を離すまでは、Escape で閉じない**（§3.6.1）。
    **phase 28 内の task_05e で直す**（task_05c で入った退行のため）。
12. **§4 の Escape 条項は「× と同じ結果で閉じる」へ言い換える**（H1。動作・コードは不変。
    × で閉じても保存される編集〔孤児ファイルの棚卸しの一覧〕は Escape でも保存される）。
13. **§4 の初期フォーカス条項へ §3.1 の保証時点を移す**（開いた時点の条項であり、
    最小化復帰後は対象外のまま = idea_34）。
14. **M2（群 A の「閉じない」検査が `focus_force` + `update` 1 回の旧形）は task_05e へ合流**する。
...
#### §3.6.1 Esc の押しっぱなし（v0.7・§2.3-11）

**規範: 別用途（記録・取得の停止）に使った Esc を離すまでは、Escape でダイアログを閉じない**。
押しっぱなしにした場合は**停止だけ**が起きて窓は残り、**一度離してから押し直した Escape で閉じる**。

- **症状（v0.6 までの実装）**: `_on_escape` は 1 発目で停止し状態を解除するため、キーリピートの 2 発目以降が
  「通常時の Escape」として扱われ、**押しっぱなしで 停止 → 閉じる**になる（ユーザーが実機で確認）。
  task_05c より前は Esc で閉じる経路が無かったので、**本フェーズの退行**。
- **実測（2026-09-23・`.venv` python。フォーカスを持つ widget の HWND へ `PostMessageW` で WM_KEYDOWN を送る probe）**:
  | 送ったもの | Tk のイベント |
  |---|---|
  | WM_KEYDOWN（初回） | `<KeyPress-Escape>` |
  | WM_KEYDOWN（repeat ビット 30 = 1）× 2 | `<KeyPress-Escape>` × 2（**KeyRelease は出ない**） |
  | WM_KEYUP | `<KeyRelease-Escape>` × 1 |
  → **Windows の Tk ではリピート中に KeyRelease が挟まらない**。「離した」は KeyRelease 1 回で判定できる
  （X11 はリピート中に KeyRelease を挟む実装があるが、本アプリの対象は Windows）。
  ※ 実キーボードのリピートではなく message の模擬。**実機目視 A3 で確認する**（§8-14）。
- **実現形**: `_on_escape` の分岐を次の**順序**で固定する（§3.6 の「**ハンドラは 1 つ・状態で分岐**」は維持）:
  1. **記録中・取得中** → 停止し、**「Esc 押下中」の印を立てて** `"break"`（**印の有無より先に判定する**）
  2. **印が立っている** → 何もせず `"break"`（**抑止するのは閉じる動作だけ**）
  3. それ以外 → 従来どおり閉じる
  **`<KeyRelease-Escape>` で印を消す**。印を持つのは停止処理を持つ 3 ダイアログ（`ActionDialog` /
  `TriggerDialog` / `KeymapEditDialog`）のみ。`PresetDialog` は Esc の別用途が無いので対象外。
  **判定順の理由（v0.7 の敵対的レビュー指摘）**: 印を先に見ると、離したことを取りこぼして印が残ったまま
  記録 / 取得をボタンで再開した場合に、**再開後の Esc が停止に届かない**。
- **離したことを取りこぼした場合**（押したままフォーカスが外へ移り、外で離した等）は、**記録 / 取得中でない
  ときの**次の Esc 押下が 1 回だけ無視され、その KeyRelease で印が消えて以降は通常どおり閉じる。
  記録 / 取得中の Esc は印の有無によらず停止する。**受容**（閉じられなくなること・止められなくなることはない）。

---

## §4 正本への追記（`features.md`「モーダルダイアログの作法」）

**3 条項**を追記する（作法の変更であり、実装だけに置くと再び正本の空白になるため）。
**v0.7 で文言を改訂**（§2.3-12・§2.3-13）。
...
>   （OK・実行などの確定操作は行わない。× で閉じても保存される編集〔「孤児ファイルの棚卸し…」の
...
正本にもその旨を 1 行残す（最小化の条項の「モーダル性は戻る」をフォーカスまで含むと読ませないため）。
...
14. **Esc の押しっぱなしで閉じない**（v0.7・§3.6.1。task_05e）:
    ①`ActionDialog`（記録中）/ `TriggerDialog` ・ `KeymapEditDialog`（取得中）の 3 経路で、
    **Escape（停止）→ Escape（リピート相当・KeyRelease なし）では窓が残り**、
    **`<KeyRelease-Escape>` の後の Escape で閉じる**ことを自動テストで固定する
    （フック停止の解除は閉じたときにちょうど 1 回）。**印が残ったまま（KeyRelease を送らずに）記録 / 取得を再開しても、
    再開後の Escape で停止する**（窓は残る）ことも固定する（§3.6.1 の判定順）。
    ②群 A の「閉じない」検査（`tests_ui/test_dialog_escape_binding.py` の `_assert_escape_stops_without_closing`）の
    送り方を、`send_escape` と同じ**期限つきのフォーカス確保**へ揃える（`focus_force` + `update` 1 回の旧形を残さない。
    `tests_ui/escape_delivery.py` のフォーカス確保部分を公開して再利用する）。
    ③**実機目視 A3**: 3 経路で「記録 / 取得中に Esc を押しっぱなし → 止まるだけで窓は残る →
    離してもう一度 Escape → 閉じる」をユーザーが確認する。
