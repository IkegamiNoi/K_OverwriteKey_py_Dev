...
モーダルを開く際に **開く前の grab 保持者を記録し、閉じたらその保持者へ戻す**。規約は次の 5 点。
...
6. **初期化の失敗を回収する** — `grab_set()` 以降の初期化で例外が出た場合、
   **子側で破棄 → 復元 → 再送出**する。破棄フックは自動では発火しないため、
   これを書かないと grab を握ったままの子が残る。
   該当する実コード: `dialogs/action_dialog.py:123`（grab 取得後に `_sync_capture_ui()`）/
   `dialogs/preset_dialog.py:39`・`action_dialog.py:120`（grab 取得後に残る `transient`）。
...
- **注意**: 案 X は破棄フックで復元するため、**`grab_set()` 後の初期化で例外が出た場合は
  自動発火しない**（`__init__` を抜けていないため）。§3-6 の明示的な回収が必須になる。
...
- **初期化失敗**（§3-6）: `grab_set()` 直後と後続初期化の 2 点で例外を注入し、
  破棄・復元・再送出を見る。
...
§6 がすべて確定し、敵対的レビューの指摘も反映したので凍結する（v0.4・2026-09-10）。
...
7. **初期化失敗**: `grab_set()` の直後および後続の初期化で例外を注入したとき、
   子が破棄され grab が元の保持者へ戻り、例外がそのまま送出される（§3-6）。
