...
### 検討事項 A: runtime を新規化・置換する入口の一本化（**未確定・v0.3 で追加**）

**由来**: Phase γ 完了時の `/refactor_check` からの候補送り。挙動保存の範囲を超えるため計画06 では見送り、
**プリセットで 2 例目が出る本フェーズで設計する**とユーザーが判断した（2026-08-06）。
...
守られている**（5 個目の潜在的な穴）。この規約は **task_07b の指摘 A（Import 経路の注入漏れ）で 1 度破れている**。

**本フェーズで問題になる理由**: プリセットの供給が hook キーと**同型**になるため、規約を続けると
**4 経路 × 2 種類**へ増える。次の注入漏れは「プリセットが復活しない」形で出る。

**決めること**:

1. 生成の入口自体が供給済みの runtime を返す形（例: `new_runtime_data(*, config_root)`）へ寄せるか、
   現状どおり「生成 + 注入」の 2 段を各所で呼ぶ規約を維持するか
2. **通常読込（`build_runtime_data_from_split`）との等価性**をどう保つか。
   通常読込は `new_default_data()` の後に**フラグ次第の条件付き注入**を行うため、前倒しの無条件注入とは
   単純には等価にならない（ON かつ keymap_set にキーが無い場合の扱い等）
3. `app.py:77` の生成を新方式へ寄せるか、起動シーケンスの前提を明文化して据え置くか

**制約**: 決定は **phase 08 の task_03 でユーザー確定**し、確定後に本書を v0.4 へ改訂してから実装する
（設計先行の不変原則）。確定内容が「現状維持」でも構わない（その場合は根拠を本書へ残す）。
