...
- **keymap_set の旧キー `hotkey_presets_path` を個別パスとして再利用**し、
  ...
  **（v0.8）フラグキーを持たないファイルは残置している `hotkey_presets_path` も引き継がない**
  （旧グローバルパスが初回 ON の保存先になっていたため）。
- **書き手はプリセットマネージャの 1 本のまま**（保存先が切り替わるだけ）。
  **保存カスケードはプリセットの内容を書かない**。
- **個別が読めなければグローバルへフォールバック**。**Import は個別指定を強制 OFF**。
- **切替 UI はマネージャ内**。**（v0.8）トグルは保存先と一覧を同時に切り替える**
  （切替先から読み直す。編集済みなら破棄確認）。**OFF へ戻した OK も表示中の一覧をグローバルへ書く**
  （グローバルが読めなければ**組込既定**へ差し替わるため、個別の内容は流出しない）。
...
- **解決順序**（個別 → 読めなければグローバル → 置き換えない）と **config 外パスの扱い**
  （**v0.8: 読み出しは許容 / 書き込みは管理下の既定パスへ寄せる**）
- **移行規則**（フラグを持たない既存 keymap_set は常に OFF・**v0.8: 残置パスも引き継がない**）
...
| task_07g | **上書き確認を 3 択にする**（上書きする / **既存を読み込む** / キャンセル。暫定仕様 **v0.10【S】**・受入条件 21。実機目視 項目 17 = 既存の内容を採る手段が無かった） |
...
- **後方互換（最重要）**: **フラグを持たない既存 keymap_set が常に OFF** になるか
  （残置 `hotkey_presets_path` を個別指定として復活させていないか）。フラグの判定は**値**で行っているか。
  **（v0.8）残置パスが「保存先」としても復活していないか**（判定はフラグキーの**有無**。
  **false + キーありのパス保持〔【N】〕を巻き添えにしていないか**）。
- **書き手の一本化**: プリセットの**内容を決める書き手がマネージャだけ**のままか。
  別名保存時の処理が**ファイルの複製に留まり**、runtime の内容を書き出していないか。
- **グローバルの保護**: 個別ファイルの保存先がグローバルと衝突しないか。
  **（v0.8）OFF へ戻した OK で書かれる内容が「グローバル自身の内容 / 組込既定」に限られる**か
  （**個別の内容が流出しない**。トグル直後・再オープン後の両方で確認する）。
- **入口台帳との整合**: E1〜E4 が常にグローバル・E5 が強制 OFF になっているか。
  **プリセット単独の注入 API を作っていない**か（正本 §5.8.8 の注記と衝突するため）。
- **パスの扱い**: 個別パスが §5.7 の表記で保存され、**config 外は書き込み側でだけ弾かれる**か
  （v0.8。読み出しは許容）。
