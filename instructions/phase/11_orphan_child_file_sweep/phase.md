...
5. **task_05 隔離** — 隔離ルートの遅延作成、**マニフェストの原子書込み（移動前）**、相対構造を保った移動、
   ...
   **`ReferenceCleanupDialog` のヘッダ / ボタンラベル引数化もここ**（「隔離する」ボタンが必要なため
   ...
   - **task_05b 隔離の堅牢化**（枝番・敵対的レビュー 2 本の指摘反映）— 候補列挙と移動直前の
     **実体（realpath）境界検証**〔ジャンクション対策〕/ ディレクトリ・リンクを移動しないガード /
     実行単位ディレクトリ作成失敗の中止化 / `exist_ok=False` / `manifest.json.tmp` の残骸除去 /
     再判定時の警告の結果への反映 / 結果表示の是正。
6. **task_06 隔離の管理（一覧 + 復元）** — 「隔離の管理…」メニューと実行単位のリスト選択 UI、
   復元（`original_path` のガード〔**同一パスの穴も塞ぐ**〕・同名スキップ・全件復元時の後始末）。
   **復元は `state` ではなく実体の有無で判定する**（task_05 の H-1 を吸収する確定判断）。
   `ReferenceCleanupDialog` は **task_05 で引数化済みのものを使うだけ**（再改修しない）。
   - **task_06b 隔離済みの削除**（枝番）— 削除（**実行単位 ID + 4 検証**・再帰削除・
     **不可逆の明記**・削除前の全件提示）と、管理ダイアログへの削除ボタン追加。
     **このフェーズ唯一の不可逆操作**のため、**復元を先に green にしてから**着手する
     （§4-F の「先に green にしてから進む」段取りに倣った task 分割）。
...
7. **task_07 統合確認** — テストスイート全体 + smoke を `verifier` で実測し、
   **実機目視の観点リストを作成**してユーザーへ提示・結果を受領。
   - **task_07b 二次レビュー指摘の反映**（枝番）— `deep-reviewer` / `codex-reviewer` で採用が
     確定した 7 件（A-1〜A-7）。参照側 symlink の無警告 skip / 隔離ルートのリダイレクト /
     ディレクトリ列挙失敗 / 既定ディレクトリの警告混入 / ダイアログ文言 / `.tmp` 残骸 /
     境界判定の二重実装。**仕様判断が要る B 群は task_08**。
