...
- 主入力（暫定仕様）: [25_trigger_list_per_keymap.md](../../history/25_trigger_list_per_keymap.md)（v0.7・ユーザー確定済〔2026-09-27 完了判定前レビューで改訂〕）。
...
- task_05: 入力判定と重複（暫定 §7: 優先順位の入れ替え / 編集時の拒否 / トリガー・キーマップのグレー表示 / キーボード表示 /
  開始検証の縮小）+ 重複キーの案内（§8.5）— **完了**（2026-09-25。重なり判定は `application/key_overlap.py` に一元化）
- task_06: キーマップ管理 UI（暫定 §8.1〜§8.3: 選択 = アクティブ化・選択ボタン削除 / 追加・編集・削除の規則 / 連続実行中の切替禁止 / 再描画 /
  シーケンスの実行位置・選択行をトリガー一覧の実体ごとに保持〔§3.3 末尾〕）— **完了**（2026-09-25。選択ボタン削除でキーマップ枠の要求幅は不変〔Codex 報告〕）
- task_07: 一時停止の改名（暫定 §8.4）— **完了**（2026-09-26。フック停止中は「（フック停止中）」・一時停止中は「（一時停止）」に分けて表示）
- task_07b: 実機目視の指摘の反映（暫定 v0.5 §2-21・§2-22: 一時的な案内をステータスバーの一時メッセージ領域へ / 追加フローのダイアログを入力エラーで閉じない）— **完了**（2026-09-26）
- task_07c: 統合レビュー A の不具合修正（H-1 選択行・実行位置のキー / M-1 / M-2 / C-2 / テスト追加 / BOM）— **完了**（2026-09-26）
- task_07d: 暫定 v0.6 の仕様変更（トリガー優先へ戻す / 移行できない旧一覧を新キーマップへ / 移行対象は項目の無いキーマップのみ / 移行先削除 = 関係解除 / 二重読込禁止 / 重なり判定の表化）— **完了**（2026-09-27）
- task_07e: 完了判定前レビューの修正と v0.7（読込後の実行位置初期化 / 共有に加わった一覧の親参照 / `""` でも新キーマップへ移行 / 移行した trigger_set の保存しない不可 / 切替中の受け付け停止）
