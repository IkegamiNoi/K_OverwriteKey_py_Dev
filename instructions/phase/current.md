...
- **アクティブなフェーズ = [phase 28](28_dialog_keyboard_focus/phase.md)**
  ...
  **進捗: task_01〜task_04 完了**（`grab_modal` へ `focus` 引数を集約・群 A/A' の 7 経路移行 /
  実 Tk の初期フォーカス検査 8 件 / 群 C の 5 経路へ Escape 結線 / **idea_18 解消**。
  `tests_ui` 484 → 502・**一括 3/3 green**）。**実装は完了済で、残りは検証と正本反映**。
  **次 = task_05**（統合確認 + 実機目視）。
