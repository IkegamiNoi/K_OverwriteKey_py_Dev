...
- **アクティブなフェーズ: [14_nested_modal_grab_restore](14_nested_modal_grab_restore/phase.md)**
  （2026-09-10 起票・**進行中**。**task_01〜04 完了 → 次は task_05**）。ネストしたモーダルを閉じたときに親の grab が復元されず、
  ...
  **presentation 層のみ・スキーマ不変・機能追加ではない**。
  ...
  - 進捗: **task_01〜04 と task_05b 完了**（ヘルパ新設 + 全 13 箇所への適用 + ネスト経路 3 系統の
    テスト + 契約 3 条のテスト + 二次レビュー指摘の反映）。**task_05 は実機目視の結果待ちで保留中**。
    自動確認は全て green（`tests` 417 / `tests_ui` 306 / smoke）。
