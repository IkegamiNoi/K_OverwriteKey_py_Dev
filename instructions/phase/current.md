...
- **アクティブなフェーズ: [14_nested_modal_grab_restore](14_nested_modal_grab_restore/phase.md)**
  （2026-09-10 起票・**進行中**。**task_01〜03 完了 → 次は task_04**）。ネストしたモーダルを閉じたときに親の grab が復元されず、
  ...
  **presentation 層のみ・スキーマ不変・機能追加ではない**。
  ...
  - 進捗: **task_01**（`presentation/modal.py` 新設 + 系統 B の 4 箇所）/ **task_02**（系統 A の
    `dialogs/` 9 クラス）/ **task_03**（ネスト経路 3 系統のテスト）**完了**。
    **適用は全 13 箇所で完了**し、ネスト経路の回帰テストも入った。
    次は **task_04**（契約 3 条のテスト = 初期化失敗 / 非 LIFO 終了 / コールバック例外）。
