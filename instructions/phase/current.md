...
- **アクティブなフェーズ = [phase 28](28_dialog_keyboard_focus/phase.md)**
  ...
  **進捗: task_01〜task_03 完了**（2026-09-22。`grab_modal` へ `focus` 引数を集約・群 A/A' の 7 経路移行 /
  実 Tk の初期フォーカス検査 8 件 / 群 C の 5 経路へ Escape 結線。`tests_ui` 484 → 502）。
  **次 = task_04**（idea_18 = Escape 配送の診断とヘルパ）。
  **一括実行は既知 flaky で不安定**（緑率 1/3。単独では安定）＝ task_04 の対象。
