...
> **ユーザー承認済（2026-08-16）→ 「計画07」として実施し完了**（末尾「計画07 の完了」節）。
...
## 項目 0: 安全網の確認（先行・必須）→ **完了（2026-08-16・`a175828`。調査 = `Explore` / 特性テスト 6 本追加）**
...
## 項目 1: `PresetManagerDialog.__init__` の UI 構築を抽出する（M2）→ **完了（2026-08-16・`f6f568d`）**
...
## 項目 2: `dialogs.py` を `dialogs/` パッケージへ分割する（M1）→ **完了（2026-08-16・`bd3321f`）**
...
## 項目 3: `global/` ディレクトリ作成の直値を定数由来にする（M6）→ **完了（2026-08-16）**
...
**項目 0〜3 のすべてを完了**（1 項目 = 1 コミット）。**挙動保存**を維持した。

| 項目 | 結果 | コミット |
|---|---|---|
| 0 | 安全網の特性テスト **6 本追加**（`tests_ui` 223 → **229**）| `a175828` |
| 1 | `PresetManagerDialog.__init__` 99 行 → **8 行**（4 メソッドへ抽出）| `f6f568d` |
| 2 | `dialogs.py` 1026 行 → `dialogs/` **7 ファイル**（最大 386 行）| `bd3321f` |
| 3 | `ensure_split_config_dirs` の直値を定数由来へ | 本コミット |

- **最終実測**: compile clean / `tests` **238** / `tests_ui` **229** / smoke pass /
  **循環 import なし** / production の import は**無変更で通る**。
- 各項目とも `verifier`（実測）+ `reviewer`（別視点）を通し、**すべて「完了可」**。
- **メトリクスの解消状況**: M2（`__init__` 99 行）= 解消 / M6（直値）= 解消 /
  M1 のうち `dialogs.py` 1016 行 = 解消（最大 386 行）。
  **`config_service/__init__.py`（734 行）は本計画の対象外**（`current.md` の「別タスク化候補」で追跡）。
- **本計画自体が `/refactor_check` の産物**のため、完了時の `/refactor_check` は実行しない。
- 判断履歴は `.claude_data/state/decisions.md` の「計画07」節。
